...
from typing import Any, Concatenate, TYPE_CHECKING, TypeAlias
...
class _CacheIntf(ABC):
    ...
    def _make_record_wrapper(
        ...
        ischema: context.IsolationSchema | None = None,
        custom_params_encoder: Callable[P, Any] | None = None,
        custom_result_encoder: Callable[[Concatenate[Any, P]], Any] | None = None,
        custom_result_decoder: Callable[[R, ...], R] | None = None,
    ) -> Callable[P, R]:
        pass
            ...
            try:
                ...
                if imc_get:
                    if custom_result_decoder:
                        return impls.Hit(
                            value=custom_result_decoder(imc_get.value, params)
                        )
                    else:
                        return imc_get
                else:
                    ...
                    if odc_get:
                        if custom_result_decoder:
                            return impls.Hit(
                                value=custom_result_decoder(odc_get.value, params)
                            )
                        return odc_get
                return None
            ...
            try:
                ...
                if imc_get:
                    ...
                    if sc_get:
                        if custom_result_decoder:
                            return impls.Hit(
                                value=custom_result_decoder(sc_get.value, params)
                            )
                        return sc_get
